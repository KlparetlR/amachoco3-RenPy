...
define gui.idle_small_color = '#6a3615'

## 当按钮和滑条被悬停时使用的颜色。
define gui.hover_color = '#ff75b1'

## 当文本按钮被选中但非焦点时使用的颜色。当一个按钮为当前屏幕或设置选项值时，
## 会处于选中状态。
define gui.selected_color = '#6a3615'
...
define font_options = {
    "思源黑体": "font/SourceHanSansSC-Bold.otf",
    "猫啃什锦体": "font/MaokenAssortedSans-Lite.otf",
    "萝莉体": "font/LoliSC.ttc"
}

#    定义一个持久化变量来存储玩家选择的字体路径
#    这保证了玩家下次启动游戏时字体设置依然有效。
#    如果 persistent.chosen_font 不存在，则默认为“萝莉体”的路径。

default persistent.chosen_font = font_options["萝莉体"]

#    将 gui.text_font 等变量指向玩家选择的字体路径
#    这样，所有使用这些 gui 变量的文本都会使用玩家选择的字体。

define gui.text_font = persistent.chosen_font

define gui.name_text_font = persistent.chosen_font

define gui.interface_text_font = persistent.chosen_font
...
## 存档位按钮。
define gui.slot_button_width = 312
...
## 存档所用缩略图的宽度和高度。
define config.thumbnail_width = 312
define config.thumbnail_height = 175
...
## 用作快进界面部分的框架。
define gui.skip_frame_borders = Borders(24, 10, 200, 10)

## 用作通知界面部分的框架。
define gui.notify_frame_borders = Borders(24, 10, 200, 10)
