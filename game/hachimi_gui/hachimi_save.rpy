init python:
    def get_page_group_from_current_page():
        try:
            current_page = int(persistent._file_page)
            return (current_page - 1) // 10
...
screen hachimi_save():
    ...
    fixed:
        ...
        grid 5 2:
            ...
            for i in range(5 * 2):
                $ slot = i + 1
                button:
                    action [Play("sys_sound_2","gui/hachimi_gui/sound/sys_sound_2.ogg"),FileSave(slot)]
                    ...
                    if _last_say_what:
                        if len(_last_say_what) > 20:
                            action [SetVariable("save_name", "\n" + re.sub(r'\{.*?\}', '', _last_say_what[:20])+"…"), FileAction(slot)]
                        else:
                            action [SetVariable("save_name", "\n" + re.sub(r'\{.*?\}', '', _last_say_what)), FileAction(slot)]
                    else:
        ...
        hbox:
            ...
            xalign 0.565
            yalign 0.032
            spacing (33 if page_group == 0 else 12)   # 直接用表达式
    ...
    hbox:
        ...
        spacing 50
        imagebutton:
            idle "gui/hachimi_gui/under_button/under_save_2.png"
        imagebutton:
